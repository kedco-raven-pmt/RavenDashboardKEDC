...
const StateMapBoxDataCards = ({ title, value, chartData, stateName }) => {
  ...
  const optionscolumnchart = {
    ...
    title: {
      text: '',
      align: 'center',
      style: { fontSize: '12px', fontWeight: 'bold', color: '#263238', padding: '10px' },
    },
  };

  const seriescolumnchart = [{ name: '', data: chartData }];
      ...
      <CardContent sx={{ p: '30px' }}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            ...
            <Box mt={2}>
              <Chart options={optionscolumnchart} series={seriescolumnchart} type="bar" height="70px" />
            </Box>
          </Grid>
        </Grid>
      </CardContent>
